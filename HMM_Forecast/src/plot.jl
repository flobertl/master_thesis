...
function plotPIT(pits::Vector{Float64}, header = "")
    # Plot
    histogram(
        pits,
        bins = 0:0.1:1,
...
function plotPITHistogram((observationSpace, infoBins), observations, distributionForecastVector::Vector{Vector{Float64}}, header = "")
    ...
    for t in 1:N
        ...
        if pits[t] > 1
            println("ACHTUNG: $(pits[t]) at time instant $N. Data: obs: $(observations[t])  \n)")
        end
    end
...
function plotPITHistogramFromQuantilesForecast(observations::Vector{Float32}, quantilesForecast::Vector{Vector{Float64}}, header = "")
    N = length(quantilesForecast)
    pits = zeros(Float64, N)
    quantiles = 0.01:0.01:1 |> vec

    for t in 1:N
        quantileIndex = searchsortedfirst(quantilesForecast[t], observations[t])
        pits[t] = quantiles[quantileIndex]
    end

    # edges = 0:0.1:1
    # h = fit(Histogram, pits, edges)
    # max_height = maximum(h.weights)
    plt = plotPIT(pits, header)
    return plt
end
...
function plotHyperparameterAnalysisCRPS(hh, tableResultsVector, hyperparameterVector::Vector{Int}, numberOfStatesVector::Vector{Int}, hyperparameterName::String)
    ...
    plt = plot(legendPlt, subplt[1], subplt[2], 
            suptitle = "Hyperparameter Tuning for HH$hh",
