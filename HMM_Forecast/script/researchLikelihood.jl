...
## Research incomplete PIT Histogramms
hmmsSummer(N) = HMM_Forecast.loadHMM("seasonmodel_hh(1)//summer//seasonmodel_states($N)")
...
testData =  observations[dateIndeces[3,6]:dateIndeces[3,9]-1]
trainDataAsIndeces = observationsAsIndeces[dateIndeces[2,6]:dateIndeces[2,9]-1]
testDataAsIndeces =  observationsAsIndeces[dateIndeces[3,6]:dateIndeces[3,9]-1]

folderPath = ".//HMM_Forecast//tmp//seasonmodel_hh(1)//summer//seasonmodel_states"

HMM_Forecast.calcTestingRoutine(hmm |> HMM_Forecast.updateHMMNumericalStable, trainData, testData, folderPath*"($N)")


distributionForecastVector = HMM_Forecast.createSeveralOneStepPredictions(hmm |> HMM_Forecast.updateHMMNumericalStable, trainDataAsIndeces, testDataAsIndeces[1:5]);
