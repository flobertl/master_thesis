...
hmm = HMM_Forecast.runBWAlgoWithRandomInit((observationSpace, dataTrainingAsIndeces), 1, 1);
...
for N in states
    ...
    HMM_Forecast.saveHMM(hmm, "basismodel_hh($hh)//states($N)//basismodel_states($N)")
    # N = 50
    ...
    push!(carme, N => HMM_Forecast.calAverageRelativeMeanError(observationSpace, dataTests, distributionForecastVector))
end
